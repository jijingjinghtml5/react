 ...
 */
Tool.header = function(header){
    header = header || {};
    //小应用特殊头
    header['X-Requested-isWEBAPP'] = 'YES';
    //session_id , vmc_uid 处理
    // var session_id = Tool.localItem('_SID'),
    //     vmc_uid = Tool.localItem('_VMC_UID');
    // SID
    // if (session_id)
...
Tool.ajax = function (mySetting) {
    ...
    try {
        if (setting.type == 'GET') { //get方式请求
            ...
            xhr.setRequestHeader('X-Requested-isWEBAPP', 'YES');
            var session_id = Tool.localItem('_SID'),
                vmc_uid = Tool.localItem('_VMC_UID');
            if (session_id)
                xhr.setRequestHeader('X-WxappStorage-SID', session_id);
            if (vmc_uid)
                xhr.setRequestHeader('X-WxappStorage-VMC-UID', vmc_uid);
            xhr.send();
        } else { //post方式请求
        ...
        if (xhr.readyState == 4) {
            ...
            if (xhr.status == 200) {
                if(response&&response.headers){
                    var x_wxappstorage = response.headers['x-wxappstorage'] || false;
                    if (x_wxappstorage) {
                        var kv = x_wxappstorage.split('=');
                        if (kv[0] && kv[1]) {
                            Tool.localItem(kv[0], kv[1]);
                        } else if (kv[0]) {
                            Tool.removeLocalItem(kv[0]);
                        }
                    }
                }
